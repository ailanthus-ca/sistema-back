		...
		$.ajax({
        type: "GET",
        url: "./ajax/cliente/buscar_clientes.php?action=eliminar",
        data: "id="+id,"q":q,
		 beforeSend: function(objeto){
		...
		function activar (id)
		{
			console.log(id);
			var q= $("#q").val();
		if (confirm("¿Deseas activar este cliente?")){	
		$.ajax({
        type: "GET",
        url: "./ajax/cliente/buscar_clientes.php?action=activar",
        data: "id="+id,"q":q,
		 beforeSend: function(objeto){
			$("#resultados").html("Mensaje: Cargando...");
		  },
	...
	function editar_cliente(id){
		$("#carga").fadeIn('slow');
		$.ajax({
			url:"./ajax/cliente/ver_editarcliente.php?id=" + id,
			 beforeSend: function(objeto){
	...
	function ver_cliente(id){
		$("#carga").fadeIn('slow');
		$.ajax({
			url:"./ajax/cliente/ver_cliente.php?id=" + id,
			 beforeSend: function(objeto){
			 $('#carga').html('<img src="./public/imagenes/ajax-loader.gif"> Cargando...');
		  },
			success:function(data){
	...
	$( "#form_editar_cliente" ).submit(function( event ) {
	 ...
	 var parametros = $(this).serialize();
		 $.ajax({
				...
				var obj = document.getElementById("btn_modalEditarCliente");
				obj.click();					
				load(1);
			  }
		});
