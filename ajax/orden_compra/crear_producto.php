...
if (isset($_POST['codigo_producto'])){$id=$_POST['codigo_producto'];}
$cantidad = 1;
//if (isset($_POST['cantidad'])){$cantidad=$_POST['cantidad'];}
if (isset($_POST['costo'])){$costo_compra=$_POST['costo'];}
...
if (!empty($id) and !empty($cantidad) and !empty($costo_compra))
{
	...
	$sql=$con->query("select *from tmp_ord_prod where id_producto = '$id'");
	if ($row=$sql->fetch_array())
	{
		$con->query("update tmp_ord_prod set cantidad_tmp = '$cantidad', precio_tmp = $costo_compra WHERE id_producto = '$id'");
	}
	else
	{
		$insert_tmp=$con->query("INSERT into tmp_ord_prod (id_producto,cantidad_tmp,precio_tmp) VALUES ('$id','$cantidad','$costo_compra')");
	}
...
<?php
	...
	$sql=$con->query("select *from producto, tmp where producto.codigo=tmp.id_producto");
