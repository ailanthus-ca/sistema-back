...
elseif ($mes!='')
{
    $query = "SELECT factura.codigo AS codigo ,cliente.nombre AS nombre, fecha, total from factura,cliente WHERE month(fecha) = $mes AND cliente.codigo = factura.cod_cliente AND usuario=$user AND `factura`.`estatus` = 2";
    switch ($mes){
        case 1:
            $m = "ENERO";
            break;
