...
require_once(dirname(__FILE__).'/../html2pdf.class.php');
    ...
    /*
     *
SELECT detallefactura.codFactura AS codigo,
    producto.descripcion AS descripcion,
    SUM( detallefactura.cantidad ) AS cantidad,
    SUM( detallefactura.monto ) AS monto
FROM detallefactura
LEFT JOIN producto ON producto.codigo = detallefactura.codProducto
INNER JOIN factura ON factura.codigo = detallefactura.codFactura
WHERE MONTH( factura.fecha ) =2
AND YEAR( factura.fecha ) =2018
AND factura.estatus =2
GROUP BY (
detallefactura.codFactura
)
     *
     */